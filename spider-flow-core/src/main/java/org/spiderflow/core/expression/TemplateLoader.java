...
import org.spiderflow.core.expression.parsing.Parser.ParserResult;
		...
		protected Template compileTemplate (Source source) {
			...
			for (Include include : result.getIncludes()) {
				...
				try {
					...
					org.spiderflow.core.expression.Error.error("Couldn't load included template '" + includePath + "'.",
						include.getSpan(), t);
				}
			}
